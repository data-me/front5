 ...
 methods: {
    ...
    saveId2 (id) {
      this.dataScientistId = id
       ...
       this.$http.post('https://api4-datame.herokuapp.com/api/v1/check_submition', formAccept, { headers:
      { Authorization: token }
      }).then((result) => {
    ...
    senderId: function(id){
      ...
      getOffer(offerId){
    var token = 'JWT ' + this.$cookies.get('token')
    this.$http.get('https://api4-datame.herokuapp.com/api/v1/offer?offerId=' + offerId,{ headers:
      { Authorization: token }
      }).then((result) => {
         ...
         this.$bvModal.msgBoxConfirm(text, {
           ...
           cancelTitle:this.$t('cancel')
            ...
            if(value === true){
              ...
              this.$http.post('https://api4-datame.herokuapp.com/api/v1/accept', formAccept, { headers:
              { Authorization: token }
              }).then((result) => {
     ...
     deleteApplication(applicationId, text) {
      this.$bvModal.msgBoxConfirm(text, {
           ...
           cancelTitle:this.$t('cancel')
            ...
            if(value === true){
              var token = 'JWT ' + this.$cookies.get('token')
              this.$http.delete('https://api4-datame.herokuapp.com/api/v2/application/' + applicationId, { headers: { Authorization: token }}).then((result) => {
     ...
     editApplication(evt) {
      ...
      body.append('description', this.applyDescription)
       ...
       this.$http.post('https://api4-datame.herokuapp.com/api/v2/application/' + this.applicationId, body, { headers:
        { Authorization: token }
        }).then((result) => {
            if (result.data.code == '200') {
